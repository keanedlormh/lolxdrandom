/**
 * client/js/game.js - CORREGIDO v1.5 (Hotfix)
 *
 * ¡FALLO CRÍTICO CORREGIDO!
 * - 1. Se ha corregido un "Race Condition". El script
 * intentaba acceder a 'canvas' y 'botones' antes de que
 * el DOM estuviera cargado.
 * - 2. Toda la inicialización (búsqueda de elementos y 
 * 'addEventListener') se ha movido DENTRO de un listener
 * 'DOMContentLoaded' para garantizar que el HTML exista.
 * - 3. Se ha revertido el error de la v1.5: `updateUI` ahora
 * vuelve a ocultar/mostrar 'canvas.style.display', no 'gameScreen'.
 */

const socket = io();

// v1.5: Declarar canvas y ctx aquí, pero ASIGNARLOS después.
let canvas, ctx;
...
// Cargar config (Se llama al inicio del script)
loadConfig();
...
function loadConfig() {
    ...
    // applyConfigToUI() y updateControlMethod() ahora se llaman
    // dentro de DOMContentLoaded, para que los elementos existan.
}
    ...
    switch(preset) {
        case 'easy':
            presetSettings = {
                ...
                initialZombies: 8, waveMultiplier: 1.2,
        ...
        case 'hard':
            presetSettings = {
                ...
                initialZombies: 12, waveMultiplier: 1.7,
                coreBaseHealth: 750, coreBaseSpawnRate: 4000,
                coreBurstSpawnMultiplier: 3.5
            };
            break;
    }
...
function interpolateEntities(factor) {
    ...
    serverSnapshot.players.forEach(p_server => {
        ...
        if (isMe) {
            clientState.amISpectating = p_server.isSpectating;
        ...
        if (p_server.health > 0 && !p_server.isSpectating) {
            ...
            clientState.cameraTargetId = p_server.id;
        }
        ...
        p_client.isSpectating = p_server.isSpectating;
    ...
    if (core_server) {
        ...
        interpolatedEntities.zombieCore = null;
    }
...
function resizeCanvas() {
    // v1.5: Solo redimensionar si el canvas ya existe
    if (canvas) {
...
// --- v1.5: CORRECCIÓN DE RACE CONDITION ---
// Todo el código que interactúa con el DOM se mueve
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Manejadores de teclado y ratón
    ...
    // --- 2. Asignar listeners de botones ---
    ...
    updateControlMethod();
    updateUI();
    // Bucle de renderizado se inicia con 'gameStarted'
});
// --- FIN v1.5 CORRECCIÓN ---


// --- Lógica de UI (se puede llamar en cualquier momento) ---
function updateUI() {
    ...
    // v1.5 CORREGIDO: Ocultar el 'canvas' (que ahora existe)
    if (canvas) canvas.style.display = 'none'; 
    // Ocultar también el contenedor por si acaso
    ...
    if (clientState.currentState === 'menu') {
        ...
        // v1.5 CORREGIDO: Mostrar el 'canvas'
        if (canvas) canvas.style.display = 'block'; 
        if (gameScreen) gameScreen.style.display = 'block'; // Y su contenedor
    } else if (clientState.currentState === 'gameOver') {
        if (gameOverScreen) gameOverScreen.style.display = 'flex';
    } else if (clientState.currentState === 'roomList') {
        if (roomListScreen) roomListScreen.style.display = 'flex';
    }
}
...
function updateLobbyDisplay() {
    ...
    if (!playerList || !startButton || !hostConfigInfo) return;
...
socket.on('joinAsSpectator', (gameData) => {
    ...
    clientState.me.isHost = false;
    clientState.playersInLobby = gameData.players;
